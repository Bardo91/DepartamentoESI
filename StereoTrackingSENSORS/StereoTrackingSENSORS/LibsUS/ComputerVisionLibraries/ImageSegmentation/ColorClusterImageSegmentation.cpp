...
namespace vision {
		...
		int ColorClusterImageSegmentation(cv::Mat& _frame, ColorClusterSpace& _CS, uint16_t _threshold, std::vector<SimpleObject>& _objects){
			...
			for (int i = 0; i < _frame.rows; i++) {
				...
				if (i) { // Except the first line that can't be child of any object (only parent) start joining objects grouped in LineObjRLE variables
					uint16_t j = 0, jp = 0; // Pointer to current and previous LineObjRLE
					uint16_t pp = aRLE[i - 1][jp].size, pc = aRLE[i][j].size; // Pointer to previous and current col
					while (1) {
						// Connecting
			...
			for (uint16_t i = 0; i < aRLE.size(); i++) {
				for (uint16_t j = 0; j < aRLE[i].size(); j++) {
	...
	//---------------- 2 Cameras
		...
		int ColorClusterImageSegmentation(	cv::Mat& _frame1, cv::Mat& _frame2, ColorClusterSpace& _CS, uint16_t _threshold, 
											std::vector<SimpleObject>& _objects1, std::vector<SimpleObject>& _objects2) {
			...
			for (int i = 0; i < _frame1.rows; i++) {
				...
				if (i) { // Except the first line that can't be child of any object (only parent) start joining objects grouped in LineObjRLE variables
					// First Camera ---------------------------------------------------------------
					uint16_t j = 0, jp = 0; // Pointer to current and previous LineObjRLE
					uint16_t pp = aRLE1[i - 1][jp].size, pc = aRLE1[i][j].size; // Pointer to previous and current col
					while (1) {
						// Connecting
			...
			// First Camera ---------------------------------------------------------------
			//Re-assign parents due to overlap
			for (uint16_t i = 0; i < aRLE1.size(); i++) {
				for (uint16_t j = 0; j < aRLE1[i].size(); j++) {
			...
			// Second Camera ---------------------------------------------------------------
			for (uint16_t i = 0; i < aRLE2.size(); i++) {
				for (uint16_t j = 0; j < aRLE2[i].size(); j++) {
			...
			for(uint16_t i = 0; i < objs1.size() ; i ++){
			...
			for(uint16_t i = 0; i < objs2.size() ; i ++){
			...
			#ifdef _DEBUG	// The images are only displayed on debug mode
